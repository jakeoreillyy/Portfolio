...
// Every icon in the site lives here so the same glyph never diverges between
// two pages. Pass `size` for the square dimensions, or a className to let CSS
// win (Tailwind's size-* overrides the width/height attributes).
type IconProps = Omit<SVGProps<SVGSVGElement>, "width" | "height" | "children"> & {
  size?: number;
};

function Outline({ size = 16, children, ...props }: IconProps & { children: ReactNode }) {
  return (
    <svg
      width={size}
      height={size}
...
function Solid({ size = 16, children, ...props }: IconProps & { children: ReactNode }) {
  return (
    <svg
      width={size}
      height={size}
